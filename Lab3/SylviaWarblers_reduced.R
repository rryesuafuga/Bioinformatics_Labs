...
library(phyloch)
sylvia.maff <- mafft(sylvia.seq, path = "C:/Users/rsseg/Downloads/mafft-win/mafft")
...
# nj.est <- drop.tip(nj.est, "Curruca_melanocephala")
# nj.est <- drop.tip(nj.est, "Curruca_conspicillata")
# nj.est <- drop.tip(nj.est, "Curruca_subcoerulea")
...
anc <- apply(ANC$lik.anc, 1, 
             FUN = function(i){
               factor(names(which.max(i)))
             })
anc <- as.character(anc)
x <- as.character(x)
X <- factor(c(x,anc), levels = c("trop","temp","temptrop"))
x <- factor(x, levels = c("trop","temp","temptrop"))
x <- factor(anc, levels = c("trop","temp","temptrop"))
X <- X[!is.na(X)]
# X <- factor(c(x, anc))
# ---- end my chnages
...
for (i in 1:nrep) {
    ...
    # X <- factor(c(x, anc)) --- my change ----
    Pvls[i] <- yule.cov(sylvia.chrono, ~ X)$Pval
}
hist(Pvls, freq = FALSE, main = "")
lines(density(Pvls))

## What is "Error in X[phy$edge[, 1], , drop = FALSE] : subscript out of bounds"
...
DF$geo.range
ape::ace(DF$geo.range, phy = sylvia.chrono, type="discrete")
ape::MPR(DF$geo.range, phy = sylvia.eco, outgroup="")
